...
func StreamReplicaSetEvents(K8 *kubernetes.K8Client, namespace string, name string, c *websocket.Conn, pollInterval int) {
	logging.INFOf("opening event stream for replica set %s/%s", namespace, name)
	...
	fieldSelector, err := fields.ParseSelector("involvedObject.name" + "=" + name)
	if err != nil {
		panic(err)
	}
	for {
