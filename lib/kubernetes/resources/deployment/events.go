...
func StreamDeploymentEvents(K8 *kubernetes.K8Client, namespace string, name string, c *websocket.Conn, pollInterval int) {
	logging.INFOf("opening event stream for deployment %s/%s", namespace, name)
	...
	for {
		events, _ := K8.K8.CoreV1().Events(namespace).List(context.Background(), metaV1.ListOptions{
			FieldSelector: "involvedObject.name=" + name, TypeMeta: metaV1.TypeMeta{Kind: "Deployment"},
		})
