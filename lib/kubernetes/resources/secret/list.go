...
func List(K8 *kubernetes.K8Client, namespace string, selector *types.DataSelector) (*types.PaginationResponse, error) {
	channelList := channels.GetSecretListChannel(K8.K8, namespace, metaV1.ListOptions{}, 1)

	secs := <-channelList.List
	...
	if selector != nil {
		comps := ToComparable(secs)
		selector.Execute(comps)
		secs = FromComparable(selector.Items)
	}

	items := []SecretMeta{}
	for _, role := range secs.Items {
		items = append(items, BuildSecretMeta(&role))
	}
