...
const server = setupServer(
    ...
    rest.get(`/api/v1/system/resources`, (req, res, ctx) =>
        res(ctx.json(generateSystemResources()))
    )
);
...
test('renders without crashing', async () => {
    const wrapper = render(
        <MemoryRouter initialEntries={['/deployments/test/test/container-spec/asdf']}>
            <Routes>
...
test('can save', async () => {
    server.use(
        rest.put(`${Deployments.base}/test/test/container-spec/asdf`, (req, res, ctx) =>
            res(ctx.json(generateDeployment('test')))
        )
    );
    const wrapper = render(
